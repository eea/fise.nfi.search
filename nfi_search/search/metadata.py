...
def comma_string_to_list(value):
    ...
    if not isinstance(value, str):
        return []
    data = [strip_or_none(f) for e in value.split(",") for f in e.split(" and ")]
...
class MetadataRecord:
    ...
    data_type = attr.ib(
        ...
        metadata={"dictionary_cls": "DDataType", "relevant": True},
    )
    data_set = attr.ib(
        converter=strip_or_none,
        metadata={"dictionary_cls": "DDataSet", "relevant": True},
    )
    resource_type = attr.ib(
        converter=strip_or_none,
        metadata={"dictionary_cls": "DResourceType", "relevant": True},
    )
    info_level = attr.ib(
        converter=strip_or_none,
        metadata={"dictionary_cls": "DInfoLevel", "relevant": True},
    )
    topic_category = attr.ib(
        converter=strip_or_none,
        metadata={"dictionary_cls": "DTopicCategory", "relevant": True},
    )
    data_source = attr.ib(
        converter=strip_or_none,
    ...
    organization = attr.ib(
        converter=strip_or_none,
        metadata={"dictionary_cls": "Organization", "relevant": True},
    )
    ...
    additional_info = attr.ib(
        converter=comma_string_to_list, metadata={"relevant": True}
    )
    keywords = attr.ib(
        converter=comma_string_to_list,
    ...
    def relevant_fields(cls):
        return [
            ...
            if field.metadata.get("relevant", False)
        ]
